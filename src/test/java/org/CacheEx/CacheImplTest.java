...
public class CacheImplTest {
    ...
    public void testLFURemoval() {
        Cache<Integer, String> cache = new CacheImpl<>(MIN_CACHE_CAPACITY, CacheReplacementPolicy.LFU);
    ...
    public void testLFUOrder() {
        ...
        cache.get(1);           //increment frequency of key 1
        cache.get(2);           //increment frequency of key 2
        cache.put(4, "four");   //the cache size is now 4 >= capacity (evict least frequently used node (3))

        //check cache items
        assertEquals("one", cache.get(1));
        assertEquals("two", cache.get(2));
        assertEquals("four", cache.get(4));
        assertNull(cache.get(3));   //check if LFU was evicted
    }
    ...
    @Test
    public void testLFUStressTest() {
        Cache<Integer, String> cache = new CacheImpl<>(MAX_CACHE_CAPACITY, CacheReplacementPolicy.LFU);
        HashSet<Integer> insertedKeys = new HashSet<>();
